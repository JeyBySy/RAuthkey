...
const Modal = ({ isOpen, onClose, children, modalTitle }) => {
    ...
    return (
        <div className="backdrop-blur-sm fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 text-white cursor-pointer " onClick={handleOutsideClick}>
            <motion.div
                initial={{ scale: 0.92 }}
                animate={{ scale: 1 }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="drop-shadow-2xl z-2 bg-elephant-900 rounded-lg shadow-lg w-[95%] sm:max-w-[500px] lg:w-[40%] lg:max-h-[50vh]  lg:absolute lg:top-[10%] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
